...
const ExperienceCard = ({ exp, isEven }: ExperienceCardProps) => {
  ...
  return (
    <section id="experience" className="py-20 bg-neutral-900 text-white">
      <div className="container mx-auto px-4 max-w-6xl relative">
        {/* Title */}
        <h2 className="text-4xl font-bold mb-20">My Career Highlights</h2>
        ...
        <div className="flex flex-col space-y-20 relative">
          {experiences.map((exp, index) => (
            <ExperienceCard key={index} exp={exp} isEven={index % 2 === 0} />
          ))}
        </div>
      </div>
    </section>
